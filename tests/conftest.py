...
# ⚠ The two header lines carrying "วันที่ :" and "วันที่จาก … ถึง …" are
# LOAD-BEARING, not decoration: parse_weekly.date_filter_is_readable() refuses a
# ขาย/ซื้อ file whose date range cannot be read (Put's policy A, 2026-08-03), and
# is_history_export() measures the reach-back from exactly these two dates. A
# fixture without them exercises the refusal path, not a weekly import. Both
# lines start with a _SKIP_PREFIXES prefix ("รหัส" / "วันที่"), so they add no
# entries — the parsed counts below are unchanged.
...
# Same as PURCHASE_SAMPLE_LINES: the "วันที่ :" + "วันที่จาก" pair is load-bearing.
SALES_SAMPLE_LINES = [
    '"(BSN)บจก.บุญสวัสดิ์นำชัย                                                                                             หน้า   :        1"',
