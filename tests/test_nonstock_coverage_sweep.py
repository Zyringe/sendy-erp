...
the exact shape file-level identity missed twice — EXPECTED_OCCURRENCE_COUNTS
records how many distinct synced_to_stock-bearing string literals the scan
currently finds per key. Both dicts are compared against a fresh scan, in
...
# finds inside each EXPECTED key's function. A NEW (or removed) literal inside
# an already-classified function changes this number and turns the sweep red
# — this is what makes identity finer than (path, function) alone, per design
# §7(a) rule 1 ("... SQL operation ..."), without requiring a 3rd dict key
# that would contradict the brief's own shown EXPECTED format.
EXPECTED_OCCURRENCE_COUNTS = {
    ('database.py', 'init_db'): 2,
    ('models/bsn_sync.py', '_sync_bsn_to_stock'): 4,
    ('models/bsn_sync.py', 'dismiss_pending_unit_conversion'): 3,
    ('models/bsn_sync.py', 'get_pending_unit_conversions'): 1,
    ('models/ecommerce_overview.py', '_sold_since_by_pid'): 1,
    ('models/imports.py', 'import_weekly'): 1,
    ('models/mapping.py', 'get_pending_split_mappings'): 2,
    ('models/reconcile.py', MODULE_LEVEL): 1,
...
def test_occurrence_count_matches_expected_per_function():
    ...
    found = _scan()
    mismatches = []
    for key, expected_count in EXPECTED_OCCURRENCE_COUNTS.items():
        actual = len(found.get(key, []))
        if actual != expected_count:
            hits = found.get(key, [])
            lines = '; '.join(f'L{ln}: {snip!r}' for ln, snip in hits)
            mismatches.append(
                f'  {key[0]}::{key[1]} — expected {expected_count} literal(s), '
                f'found {actual}: {lines or "(none)"}')
    assert not mismatches, (
        "Occurrence count drifted inside an already-classified function — a "
        "statement was added or removed. Re-read the function and update both "
        "EXPECTED's reason (if the change affects the guard) and "
        "EXPECTED_OCCURRENCE_COUNTS:\n" + "\n".join(mismatches))


def test_expected_dicts_share_the_same_keys():
    """Internal consistency: every EXPECTED entry needs a count, and vice
    versa — a maintenance slip here would silently blind one of the two
    checks above."""
    assert set(EXPECTED) == set(EXPECTED_OCCURRENCE_COUNTS), (
        set(EXPECTED) ^ set(EXPECTED_OCCURRENCE_COUNTS))
