"""Migration 159 — ar_followup_log soft delete (deleted_at / deleted_by).
...
Fixture `pre159_conn`: `empty_db` clones the LIVE LOCAL schema, which on any
machine that has already booted this branch's app carries migration 159 — so
this file must NOT assume the columns are absent. Detect and roll back first
so the fixture reconstructs the true pre-159 state either way (same shape as
...
MIG_159 = os.path.join(
    REPO, "data", "migrations", "159_ar_followup_soft_delete.sql")
ROLLBACK_159 = os.path.join(
    REPO, "data", "migrations", "159_ar_followup_soft_delete.rollback.sql")
MIG_159_NAME = "159_ar_followup_soft_delete.sql"
...
def test_forward_adds_both_columns_as_nullable_text(pre159_conn):
    before = _cols(pre159_conn)
    assert "deleted_at" not in before, "fixture failed to reconstruct pre-159 state"
    assert "deleted_by" not in before

    _apply(pre159_conn, MIG_159)

    after = _cols(pre159_conn)
...
def test_forward_creates_partial_index_over_live_rows(pre159_conn):
    _apply(pre159_conn, MIG_159)
    ...
    meta = {
        r["name"]: r
        for r in pre159_conn.execute("SELECT * FROM pragma_index_list('ar_followup_log')")
    }
...
def test_forward_index_serves_the_reader_query_and_break_it_once(pre159_conn):
    ...
    with_index = _plan(pre159_conn, READER_QUERY, params)
    ...
    pre159_conn.execute("DROP INDEX %s" % INDEX_NAME)
    without_index = _plan(pre159_conn, READER_QUERY, params)
    ...
    _apply(pre159_conn, ROLLBACK_159)
    _apply(pre159_conn, MIG_159)
    assert INDEX_NAME in _plan(pre159_conn, READER_QUERY, params)


def test_forward_keeps_the_three_original_indexes_byte_identical(pre159_conn):
    before = {
        name: sql for typ, name, sql in _master(pre159_conn) if typ == "index"
    }
    assert set(ORIGINAL_INDEXES) <= set(before), (
        "pre-159 state is missing an original index: " + repr(sorted(before)))

    _apply(pre159_conn, MIG_159)

    after = {name: sql for typ, name, sql in _master(pre159_conn) if typ == "index"}
...
def test_forward_does_not_self_stamp_applied_migrations(pre159_conn):
    ...
    stamped = pre159_conn.execute(
        "SELECT COUNT(*) AS n FROM applied_migrations WHERE filename=?", (MIG_159_NAME,)
    ).fetchone()["n"]
    assert stamped == 0, "migration 159 stamped its own applied_migrations row"
    ...
    assert pre159_conn.execute(
        "SELECT COUNT(*) AS n FROM applied_migrations WHERE filename=?", (MIG_159_NAME,)
    ).fetchone()["n"] == 1


def test_runner_applies_it_and_writes_the_stamp(pre159_conn):
    """Drive the REAL runner (database.run_pending_migrations), not a hand
    executescript: it is what decides 159 is pending, executes it, and records
    ...
    assert MIG_159_NAME in files, "159 is not visible to the runner"
    pre159_conn.executemany(
        "INSERT INTO applied_migrations (filename, applied_by) VALUES (?, 'test-preseed')",
        [(f,) for f in files if f != MIG_159_NAME],
    )
    pre159_conn.commit()

    ran = database.run_pending_migrations(pre159_conn, verbose=False)

    assert ran == [MIG_159_NAME], f"runner ran {ran!r}"
    assert "deleted_at" in _cols(pre159_conn), "runner stamped without applying?"
    ...
    assert row["sha256"] == database._file_sha256(MIG_159), (
...
def test_rollback_removes_both_columns_and_the_index(pre159_conn):
    _apply(pre159_conn, MIG_159)
    assert "deleted_at" in _cols(pre159_conn)  # control: forward really ran

    _apply(pre159_conn, ROLLBACK_159)

    cols = _cols(pre159_conn)
...
def test_rollback_preserves_rows_inserted_after_the_forward_migration(pre159_conn):
    ...
    pre_id = _insert_log(pre159_conn, "ร้านก่อน", "C-PRE", "2026-08-01", "pre-forward row")

    _apply(pre159_conn, MIG_159)

    post_id = _insert_log(
        pre159_conn, "ร้านหลัง", "C-POST", "2026-08-15", "post-forward row",
        deleted_at="2026-08-15 10:00:00", deleted_by="admin",
    )
    soft_deleted_id = _insert_log(
        pre159_conn, "ร้านลบ", "C-DEL", "2026-08-14", "soft-deleted row",
    ...
    n_before = pre159_conn.execute(
        "SELECT COUNT(*) AS n FROM ar_followup_log").fetchone()["n"]
    assert n_before == 3
    before = {
        i: _row_without_soft_delete_cols(pre159_conn, i)
        for i in (pre_id, post_id, soft_deleted_id)
    }

    _apply(pre159_conn, ROLLBACK_159)

    n_after = pre159_conn.execute(
        "SELECT COUNT(*) AS n FROM ar_followup_log").fetchone()["n"]
    assert n_after == 3, (
...
def test_rollback_keeps_the_three_original_indexes_byte_identical(pre159_conn):
    before = {name: sql for typ, name, sql in _master(pre159_conn) if typ == "index"}
    assert set(ORIGINAL_INDEXES) <= set(before)
...
def test_rollback_removes_only_this_migrations_stamp(pre159_conn):
    _apply(pre159_conn, MIG_159)
    pre159_conn.executemany(
        "INSERT INTO applied_migrations (filename, applied_by, sha256, duration_ms) "
        "VALUES (?, 'auto', 'deadbeef', 3)",
        [(MIG_159_NAME,), ("158_conversion_input_role.sql",)],
    )
    pre159_conn.commit()
    assert pre159_conn.execute(
    ...
    names = {r["filename"] for r in pre159_conn.execute(
        "SELECT filename FROM applied_migrations")}
    assert MIG_159_NAME not in names, "rollback left the runner's stamp behind"
    assert "158_conversion_input_role.sql" in names, (
        "rollback deleted a neighbouring migration's stamp — the DELETE is not scoped")


def test_rollback_index_drop_must_precede_the_column_drop(pre159_conn):
    ...
    with pytest.raises(sqlite3.OperationalError) as exc:
        pre159_conn.execute("ALTER TABLE ar_followup_log DROP COLUMN deleted_at")
    assert INDEX_NAME in str(exc.value)


def test_rollback_leaves_sqlite_master_byte_identical_to_pre_forward(pre159_conn):
    before = _master(pre159_conn)
    assert before, "no sqlite_master objects for ar_followup_log — fixture is wrong"

    _apply(pre159_conn, MIG_159)
    assert _master(pre159_conn) != before  # control: forward really changed it

    _apply(pre159_conn, ROLLBACK_159)

    assert _master(pre159_conn) == before, (
        "forward+rollback did not restore the table/index DDL byte-for-byte")


def test_forward_rollback_forward_is_clean(pre159_conn):
...
def test_no_trigger_or_view_depends_on_ar_followup_log(pre159_conn):
    ...
    matched_any = [
        r["name"] for r in pre159_conn.execute(
    ...
    assert others == [], (
        ...
        + " — re-verify migration 159's rollback (DROP COLUMN) against it")
