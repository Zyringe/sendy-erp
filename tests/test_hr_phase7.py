"""Phase 7 — Salary-advance data layer + read-only /hr/advances.

Covers: mig 119 column, the hr_queries helpers (still used internally +
by tests), and the HR nav.

NOTE: the HR advance WRITE routes (new/edit/delete) were removed in the cashbook
Phase 2 overhaul (plan.md decision C5c) — /cashbook/new is now the sole live
writer of salary_advances. Their "route gone / read-only" coverage lives in
tests/test_hr_advances_readonly.py, and the deducted-lock semantics now live on
the cashbook side (tests/test_cashbook_advance_lock_and_delete.py).
...
# ── Task 7.2 — hr_queries helpers (still used internally) ─────────────────────
...
# ── Task 7.5 — access gate: staff still blocked from the read-only page ───────
...
# ── Nav completeness — the advances tab must show in BOTH navs ────────────────
...
def test_advances_nav_present_desktop_and_mobile(tmp_db):
    ...
    # desktop: the advances page must resolve to the 'hr' module, else the whole
    # HR sidebar block ({% if active_module == 'hr' %}) disappears on that page.
