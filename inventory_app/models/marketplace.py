"""Marketplace (Shopee/Lazada) orders, settlements, fees, payout batches,
reconciliation — extracted verbatim from models.py (behavior-preserving
split, Phase 12) — see models/__init__.py's module docstring for the
overall file-split rationale. No behavior changes.

Only 4 module-level functions (get_marketplace_summary, get_marketplace_orders,
get_marketplace_unmapped, get_marketplace_returns_cancelled) use the header
`get_connection` import — 7 other functions do their own local
`from database import get_connection` inside the function body (intentional,
kept verbatim per the brief). The `from marketplace_fee_buckets import
LAZADA_BUCKET, GRANULAR_LABEL` line is likewise kept in its original
mid-body position (verbatim), directly above the `_FEE_LABELS` constant it
