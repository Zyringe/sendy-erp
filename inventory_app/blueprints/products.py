...
@bp_products.route('/products/walkthrough')
...
@bp_products.route('/api/photos/review-queue')
def api_photos_review_queue():
    ...
    for rel, _ab in files[offset:offset + limit]:
        out.append({
            'rel': rel,
            'url': url_for('products.serve_catalog_photo', filepath='_review/' + rel),
        })
